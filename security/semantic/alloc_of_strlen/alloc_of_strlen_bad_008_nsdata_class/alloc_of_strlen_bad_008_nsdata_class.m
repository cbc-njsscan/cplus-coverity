#import <Foundation/Foundation.h>
#import <string.h>

// This suite of tests uses NSData class methods which take length as an
// initialising parameter.
void test1(const char *input) {
  // We malloc here as `freeWhenDone` requires a malloc'd buffer when the
  // argument is true.
  // VULNERABILITY HERE: AllocOfStrlen
  char *buf = malloc(strlen(input));
  memcpy(buf, input, strlen(input));
  // VULNERABILITY HERE: AllocOfStrlen
  NSData *data = [NSData dataWithBytesNoCopy:buf
                                      length:strlen(input)
                                freeWhenDone:YES];
}

void test2(const char *input) {
  // malloc is required here
  // VULNERABILITY HERE: AllocOfStrlen
  char *buf = malloc(strlen(input));
  memcpy(buf, input, strlen(input));
  // VULNERABILITY HERE: AllocOfStrlen
  NSData *data = [NSData dataWithBytesNoCopy:buf length:strlen(input)];
}

void test3(const char *input) {
  // VULNERABILITY HERE: AllocOfStrlen
  NSData *data = [NSData dataWithBytes:(void *)input length:strlen(input)];
}

int main(int argc, const char *argv[]) {
  @autoreleasepool {
    const char *str = "ImportantString";
    test1(str);
    test2(str);
    test3(str);
  }
  return 0;
}